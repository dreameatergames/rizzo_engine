...
cvar_t loadas8bit = {"loadas8bit", "0"}; //Hack, maybe enable
...
void S_SoundInfo_f(void)
{
    ...
    Con_DPrintf("0x%x dma buffer\n", shm->buffer);
	Con_DPrintf("%5d total_channels\n", total_channels);
}
...
void S_Startup (void)
{
	...
	if (!fakedma)
	{
		...
		{
#ifndef	_WIN32
			Con_DPrintf("S_Startup: SNDDMA_Init failed.\n");
#endif
			sound_started = 0;
			return;
		}
...
sfx_t *S_FindName (char *name)
{
	...
	if (!name)
		Con_DPrintf ("S_FindName: NULL\n");

	if (strlen(name) >= MAX_QPATH)
		Con_DPrintf ("Sound name too long: %s", name);
...
// see if already loaded
	...
	if (num_sfx == MAX_SFX)
		Con_DPrintf ("S_FindName: out of sfx_t");
...
void S_ClearBuffer (void)
{
	...
	if (pDSBuf)
	{
		...
		while ((hresult = pDSBuf->lpVtbl->Lock(pDSBuf, 0, gSndBufSize, (void**)&pData, &dwSize, NULL, NULL, 0)) != DS_OK)
		{
			...
			{
				Con_DPrintf ("S_ClearBuffer: DS::Lock Sound Buffer Failed\n");
				S_Shutdown ();
				return;
			}
			...
			if (++reps > 10000)
			{
				Con_DPrintf ("S_ClearBuffer: DS: couldn't restore buffer\n");
				S_Shutdown ();
				return;
			}
		}
...
void S_LocalSound (char *sound)
{
	...
	if (!sfx)
	{
		Con_DPrintf ("S_LocalSound: can't cache %s\n", sound);
		return;
	}
