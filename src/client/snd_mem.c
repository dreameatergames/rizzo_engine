...
//			Con_DPrintf ("FindNextChunk: %i length is past the 1 meg sanity limit", iff_chunk_len);
		data_p -= 8;
...
// find data chunk
	...
	if (info.samples)
	{
		if (samples < info.samples)
			Con_DPrintf ("Sound %s has a bad loop length", name);
	}
	else
