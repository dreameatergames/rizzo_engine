...
// make sure LoadLibrary has happened successfully
	...
	if (winsock_initialized == 0)
	{
		...
		if (r)
		{
			Con_DPrintf ("Winsock initialization failed.\n");
			return -1;
		}
	}
	winsock_initialized++;
	...
	Con_DPrintf("Winsock IPX Initialized\n");
	ipxAvailable = true;
...
void WIPX_Listen (qboolean state)
{
	...
	if (state)
	{
		...
		if ((net_acceptsocket = WIPX_OpenSocket (net_hostport)) == -1)
			Con_DPrintf ("WIPX_Listen: Unable to open accept socket\n");
		return;
	}
...
int WIPX_OpenSocket (int port)
{
	...
	Con_DPrintf ("Winsock IPX bind failed\n");
ErrorReturn:
...
int WIPX_CheckNewConnections (void)
{
	...
	if (pioctlsocket (ipxsocket[net_acceptsocket], FIONREAD, &available) == -1)
		Con_DPrintf ("WIPX: ioctlsocket (FIONREAD) failed\n");
	if (available)
