...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
	...
	if (i)
	{
		if (i < com_argc-1)
		{
			...
			if (myAddr == INADDR_NONE)
				Con_DPrintf ("%s is not a valid IP address", com_argv[i+1]);
			strcpy(my_tcpip_address, com_argv[i+1]);
		}
		else
		{
			Con_DPrintf ("NET_Init: you must specify an IP address after -ip");
		}
	}
	else
	{
	...
	{
		Con_DPrintf("WINS_Init: Unable to open control socket\n");
		if (--winsock_initialized == 0)
			pWSACleanup ();
		return -1;
	}
...
void WINS_Listen (qboolean state)
{
	...
	if (state)
	{
		...
		if ((net_acceptsocket = WINS_OpenSocket (net_hostport)) == -1)
			Con_DPrintf ("WINS_Listen: Unable to open accept socket\n");
		return;
	}
...
int WINS_OpenSocket (int port)
{
	...
	Con_DPrintf ("Unable to bind to %s", WINS_AddrToString((struct qsockaddr *)&address));
ErrorReturn:
...
int WINS_Broadcast (int socket, byte *buf, int len)
{
	...
	if (socket != net_broadcastsocket)
	{
		if (net_broadcastsocket != 0)
			Con_DPrintf("Attempted to use multiple broadcasts sockets\n");
		WINS_GetLocalAddress();
		ret = WINS_MakeSocketBroadcastCapable (socket);
		if (ret == -1)
		{
			Con_DPrintf("Unable to make socket broadcast capable\n");
			return ret;
		}
	}
