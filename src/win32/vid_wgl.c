...
// Who knows if it helps, but it probably doesn't hurt
	...
	vid_modenum = modenum;
	Cvar_SetValue (vid_mode.name, (float)vid_modenum);
...
void CheckMultiTextureExtensions(void)
{
   ...
   if (ARB || SGIS)
   {
      ...
      gl_Texture0 = ARB ? 0x84C0 : 0x835E;
      gl_Texture1 = ARB ? 0x84C1 : 0x835F;
      gl_oldtarget = gl_Texture0;
      gl_mtexable = true;
   }
   else
...
void	VID_Init (unsigned char *palette)
{
	...
	int		i, existingmode;
	int		width, height=0, bpp, findbpp, done;
	HDC		hdc;
	DEVMODE	devmode;
	...
	//InitCommonControls(); /* @Note: do we need? */
