...
#define CONSOLE_ERROR_TIMEOUT	60.0	// # of seconds to wait on Con_DPrintf running
...
int Sys_FileOpenWrite (char *path)
{
	...
	f = fopen(path, "wb");
	if (!f)
		Con_DPrintf ("Error opening %s: %s", path,strerror(errno));
	sys_handles[i] = f;
...
void Sys_Init (void)
{
	...
	if (!GetVersionEx (&vinfo))
		Con_DPrintf ("Couldn't get OS info");

	if ((vinfo.dwMajorVersion < 4) ||
		(vinfo.dwPlatformId == VER_PLATFORM_WIN32s))
	{
		Con_DPrintf ("Rizzo Island requires at least Win95 or NT 4.0");
	}
...
void Sys_Error (char *error, ...)
{
	...
	// tried that and failed
		if (!in_sys_error0)
		{
			in_sys_error0 = 1;
			VID_SetDefaultMode ();
			MessageBox(NULL, text, "Rizzo Error",
					   MB_OK | MB_SETFOREGROUND | MB_ICONSTOP);
		}
		else
		{
			MessageBox(NULL, text, "Double Rizzo Error",
					   MB_OK | MB_SETFOREGROUND | MB_ICONSTOP);
		}
	}
...
char *Sys_ConsoleInput (void)
{
	...
	for ( ;; )
	{
		if (!GetNumberOfConsoleInputEvents (hinput, (LPDWORD)&numevents))
			Con_DPrintf ("Error getting # of console events");

		if (numevents <= 0)
			break;

		if (!ReadConsoleInput(hinput, recs, 1, (LPDWORD)&numread))
			Con_DPrintf ("Error reading console input");

		if (numread != 1)
			Con_DPrintf ("Couldn't read console input");
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!GetCurrentDirectory (sizeof(cwd), cwd))
		Con_DPrintf ("Couldn't determine current directory");
...
// request otherwise
	...
	if (!parms.membase)
		Con_DPrintf ("Not enough memory free; check disk space\n");
