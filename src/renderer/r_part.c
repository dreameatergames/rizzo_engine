...
int		ramp3[8] = {0x6d, 0x6b, 6, 5, 4, 3, 3, 3};
...
void R_ReadPointFile_f (void)
{
	...
	for ( ;; )
	{
		...
		if (!free_particles)
		{
			Con_DPrintf ("Not enough free particles\n");
			break;
		}
		p = free_particles;
		free_particles = p->next;
...
void R_DrawParticles (void)
{
  ...
  glColorPointer(GL_BGRA, GL_UNSIGNED_BYTE, sizeof(glvert_fast_t), &submission_pointer->color);
	...
	for (p=active_particles ; p ; p=p->next)
	{
		...
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX , .vert = {p->org[0], p->org[1], p->org[2]}, .texture = {0,0}, .color = {(color >> 16) & 255, (color >> 8) & 255,color & 255, 255}, .pad0 = {0}};
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX , .vert = {p->org[0] + up[0]*scale, p->org[1] + up[1]*scale, p->org[2] + up[2]*scale}, .texture = {1,0}, .color = {(color >> 16) & 255, (color >> 8) & 255,color & 255, 255}, .pad0 = {0}};
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX_EOL , .vert = {p->org[0] + right[0]*scale, p->org[1] + right[1]*scale, p->org[2] + right[2]*scale}, .texture = {0,1}, .color = {(color >> 16) & 255, (color >> 8) & 255,color & 255, 255}, .pad0 = {0}};
