...
int Scrap_AllocBlock(int w, int h, int *x, int *y) {
  ...
  Con_DPrintf("Scrap_AllocBlock: full");
...
qpic_t *Draw_CachePic(char *path) {
  ...
  if (menu_numcachepics == MAX_CACHED_PICS)
    Con_DPrintf("menu_numcachepics == MAX_CACHED_PICS");
  menu_numcachepics++;
...
void Draw_Init(void) {
  ...
  if (!cb)
    Con_DPrintf("Couldn't load gfx/conback.lmp");
  SwapPic(cb);
...
void GL_Upload32(unsigned *data, int width, int height, uint8_t flags) {
  ...
  if (scaled_width * scaled_height > (int)sizeof(scaled) / 4)
    Con_DPrintf("GL_LoadTexture: too big");
...
void GL_Upload8_EXT(byte *data, int width, int height,uint8_t flags) {
  ...
  unsigned char *scaled = malloc(256 * 256);
...
// BlackAura - begin
  ...
  if (!xlated)
    Con_DPrintf("GL_Upload8: Can't change texture format - out of memory");
  ptr = xlated;
...
void DrawQuad_NoTex(float x, float y, float w, float h) {
  ...
  //Vertex 1
  //Quad vertex
  quadvert[0] = (glvert_fast_t){.flags = VERTEX, .vert = {x, y, 0}, .texture = {0, 0}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 4
  //Quad vertex
  quadvert[1] = (glvert_fast_t){.flags = VERTEX, .vert = {x, y + h, 0}, .texture = {0, 0}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 2
  //Quad vertex
  quadvert[2] = (glvert_fast_t){.flags = VERTEX, .vert = {x + w, y, 0}, .texture = {0, 0}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 3
  //Quad vertex
  quadvert[3] = (glvert_fast_t){.flags = VERTEX_EOL, .vert = {x + w, y + h, 0}, .texture = {0, 0}, .color = {255, 255, 255, 255}, .pad0 = {0}};
...
void DrawQuad(float x, float y, float w, float h, float u, float v, float uw, float vh) {
  ...
  //Vertex 1
  //Quad vertex
  quadvert[0] = (glvert_fast_t){.flags = VERTEX, .vert = {x, y, 0}, .texture = {u, v}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 4
  //Quad vertex
  quadvert[1] = (glvert_fast_t){.flags = VERTEX, .vert = {x, y + h, 0}, .texture = {u, v + vh}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 2
  //Quad vertex
  quadvert[2] = (glvert_fast_t){.flags = VERTEX, .vert = {x + w, y, 0}, .texture = {u + uw, v}, .color = {255, 255, 255, 255}, .pad0 = {0}};

  //Vertex 3
  //Quad vertex
  quadvert[3] = (glvert_fast_t){.flags = VERTEX_EOL, .vert = {x + w, y + h, 0}, .texture = {u + uw, v + vh}, .color = {255, 255, 255, 255}, .pad0 = {0}};
  glEnableClientState(GL_COLOR_ARRAY);
  glEnableClientState(GL_TEXTURE_COORD_ARRAY);
  glVertexPointer(3, GL_FLOAT, sizeof(glvert_fast_t), &quadvert[0].vert);
  glTexCoordPointer(2, GL_FLOAT, sizeof(glvert_fast_t), &quadvert[0].texture);
