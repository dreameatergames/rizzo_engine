...
store:
	...
	case GL_COLOR_INDEX8_EXT:
		...
		break;
	default:
		Con_DPrintf ("Bad lightmap format");
	}
}
...
texture_t *R_TextureAnimation (texture_t *base)
{
	...
	while (base->anim_min > reletive || base->anim_max <= reletive)
	{
		base = base->anim_next;
		if (!base)
			Con_DPrintf ("R_TextureAnimation: broken cycle");
		if (++count > 100)
			Con_DPrintf ("R_TextureAnimation: infinite cycle");
	}
...
void DrawGLWaterPoly (glpoly_t *p)
{
  ...
  for (i = 0; i < p->numverts - 2; i++) {
		...
		r_batchedtempverts[numwaterverts++] = (glvert_fast_t){.flags = VERTEX, .vert = {pnv[0], pnv[1], pnv[2]}, .texture = {v0[3], v0[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		r_batchedtempverts[numwaterverts++] = (glvert_fast_t){.flags = VERTEX, .vert = {tv0, tv1, v[2]}, .texture = {v[3], v[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		v += VERTEXSIZE;

		tv0 = v[0] + 8*SIN(v[1]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		tv1 = v[1] + 8*SIN(v[0]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		r_batchedtempverts[numwaterverts++] = (glvert_fast_t){.flags = VERTEX_EOL, .vert = {tv0, tv1, v[2]}, .texture = {v[3], v[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
  }
}
...
void DrawGLWaterPolyLightmap (glpoly_t *p)
{
  ...
  v = p->verts[1];
    ...
    for (i = 0; i < p->numverts - 2; i++)
    {
		...
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX, .vert = {pnv[0], pnv[1], pnv[2]}, .texture = {v0[5], v0[6]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX, .vert = {tv0, tv1, v[2]}, .texture = {v[5], v[6]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		v += VERTEXSIZE;

		tv0 = v[0] + 8*SIN(v[1]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		tv1 = v[1] + 8*SIN(v[0]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		*submission_pointer++ = (glvert_fast_t){.flags = VERTEX_EOL, .vert = {tv0, tv1, v[2]}, .texture = {v[5], v[6]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
    }
}
  ...
  glLoadMatrixf (r_world_matrix);
    ...
    glColorPointer(GL_BGRA, GL_UNSIGNED_BYTE, sizeof(glvert_fast_t), &submission_pointer->color);
		...
		for ( ; s ; s=s->texturechain){
			...
			//R_SetDirectBufferAddress(submission_pointer);
			//R_SetDirectBufferAddress(R_GetDirectBufferAddress()+((p->numverts - 2)*3))
			//printf("drew water verts %d vs %d\n", numwaterverts, numwaterverts2);
		}
		glDrawArrays(GL_TRIANGLES, 0, numwaterverts);
	...
	// draw the polygon
			...
			(!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
		{
			if (gl_texsort.value) {
				...
				//R_DrawSequentialPoly (psurf); //@Note: Slower Currently so unsupported
			}
		}
	}
			...
			side = 0;
		{
			for ( ; c ; c--, surf++)
			{
				...
				if (gl_texsort.value)
				{
					...
					{}//R_DrawSequentialPoly (surf);  //@Note: Slower Currently so unsupported
