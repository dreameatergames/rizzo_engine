...
void VID_DescribeMode_f (void)
{
	...
	if ((modenum >= num_modes) || (modenum < 0 ) || !modes[modenum].width)
		Con_DPrintf("Invalid video mode: %d!\n",modenum);
	Con_DPrintf("%d: %d x %d - ",modenum,modes[modenum].width,modes[modenum].height);
...
void VID_DescribeModes_f (void)
{
	...
	for (i=0;i<num_modes;i++)
		if (modes[i].width) {
			Con_DPrintf("%d: %d x %d - ", i, modes[i].width,modes[i].height);
...
{
	Con_DPrintf("mode: %d\n",current_mode);
	Con_DPrintf("height x width: %d x %d\n",vid.height,vid.width);
	Con_DPrintf("bpp: %d\n",modes[current_mode].bytesperpixel*8);
	Con_DPrintf("vid.aspect: %f\n",vid.aspect);
}
...
int VID_SetMode (int modenum, unsigned char *palette)
{
	...
	if ((modenum >= num_modes) || (modenum < 0) || !modes[modenum].width)
	{
		...
		Con_DPrintf("No such video mode: %d\n",modenum);
	...
	vid.colormap = (byte *) host_colormap;
	...
	vid.buffer = vid.buffer = (byte *)(((byte *)d_pzbuffer) + zsize + tsize);
...
//			framebuffer_ptr = (char *) vga_getgraphmem();
	if (!framebuffer_ptr)
		Con_DPrintf("This mode isn't hapnin'\n");
...
//		scantokey[69] = JK_NUM_LOCK;
		...
		if (keyboard_init())
			Con_DPrintf("keyboard_init() failed");
