...
static void install_grabs(void) {
  ...
  if (in_dgamouse.value) {
    ...
    if (!XF86DGAQueryVersion(dpy, &MajorVersion, &MinorVersion)) {
      ...
      Con_DPrintf("Failed to detect XF86DGA Mouse\n");
      in_dgamouse.value = 0;
    } else {
      dgamouse = true;
...
void CheckMultiTextureExtensions(void) {
  ...
  if (strstr(gl_extensions, "GL_SGIS_multitexture ") && !COM_CheckParm("-nomtex")) {
    Con_DPrintf("Found GL_SGIS_multitexture...\n");

    if ((prjobj = dlopen(NULL, RTLD_LAZY)) == NULL) {
      Con_DPrintf("Unable to open symbol list for main program.\n");
      return;
    }
    ...
    if (qglMTexCoord2fSGIS && qglSelectTextureSGIS) {
      ...
      Con_DPrintf("Symbol not found, disabled.\n");
...
void VID_Init8bitPalette(void) {
  ...
  if (strstr((const char*)gl_extensions, "GL_EXT_shared_texture_palette") &&
