...
void TragicDeath(int signal_num)
{
	...
	Con_DPrintf("This death brought to you by the number %d\n", signal_num);
}
...
// alloc an extra line in case we want to wrap, and allocate the z-buffer
	...
	if (d_pzbuffer == NULL)
		Con_DPrintf ("Not enough memory for video mode\n");
	...
	if (!x_framebuffer[0])
		Con_DPrintf("VID: XCreateImage failed\n");
...
// alloc an extra line in case we want to wrap, and allocate the z-buffer
	...
	// grab shared memory
		...
		if (size < minsize)
			Con_DPrintf("VID: Window must use at least %d bytes\n", minsize);

		key = random();
		x_shminfo[frm].shmid = shmget((key_t)key, size, IPC_CREAT|0777);
		if (x_shminfo[frm].shmid==-1)
			Con_DPrintf("VID: Could not get any shared memory\n");
	...
	// get the X server to attach to it
		...
		if (!XShmAttach(x_disp, &x_shminfo[frm]))
			Con_DPrintf("VID: XShmAttach() failed\n");
		XSync(x_disp, 0);
...
// open the display
	...
	{
		if (getenv("DISPLAY"))
			Con_DPrintf("VID: Could not open display [%s]\n",
				getenv("DISPLAY"));
		else
			Con_DPrintf("VID: Could not open local display\n");
	}
...
// check for command-line window size
	...
	{
		if (pnum >= com_argc-2)
			Con_DPrintf("VID: -winsize <width> <height>\n");
		vid.width = atoi(com_argv[pnum+1]);
		vid.height = atoi(com_argv[pnum+2]);
		if (!vid.width || !vid.height)
			Con_DPrintf("VID: Bad window width/height\n");
	}
	if ((pnum=COM_CheckParm("-width"))) {
		if (pnum >= com_argc-1)
			Con_DPrintf("VID: -width <width>\n");
		vid.width = atoi(com_argv[pnum+1]);
		if (!vid.width)
			Con_DPrintf("VID: Bad window width\n");
	}
	if ((pnum=COM_CheckParm("-height"))) {
		if (pnum >= com_argc-1)
			Con_DPrintf("VID: -height <height>\n");
		vid.height = atoi(com_argv[pnum+1]);
		if (!vid.height)
			Con_DPrintf("VID: Bad window height\n");
	}
...
// pick a visual- warn if more than one was available
	...
	else if (num_visuals == 0)
	{
		if (template_mask == VisualIDMask)
			Con_DPrintf("VID: Bad visual id %d\n", template.visualid);
		else
			Con_DPrintf("VID: No visuals at depth %d\n", template.depth);
	}
		...
		while (rects)
		{
			...
			if (!XShmPutImage(x_disp, x_win, x_gc,
				...
				rects->x, rects->y, rects->width, rects->height, True))
					Con_DPrintf("VID_Update: XShmPutImage failed\n");
			oktodraw = false;
