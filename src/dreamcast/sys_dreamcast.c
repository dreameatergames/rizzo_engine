...
int Sys_FileOpenWrite(char *path) {
  ...
  f = fopen(path, "w");
  if (f == NULL)
    Con_DPrintf("[%s] Error opening %s: %s", __func__, path, strerror(errno));
  sys_handles[i] = f;
...
void Sys_Quit(void) {
  ...
  drawtext(32, 64, "RizzoDC shutdown...");
  arch_exit();
}
...
static void assert_hnd(const char *file, int line, const char *expr, const char *msg, const char *func) {
  ...
  /* Display the error message on screen */
  drawtext(32, 64, "RizzoDC - Assertion failure");
...
//Load Directly
	...
	if (basedir==NULL)
		Con_DPrintf("can't find quake dir");
  ...
  if (!parms.membase)
    Con_DPrintf("Not enough memory free;\n");
