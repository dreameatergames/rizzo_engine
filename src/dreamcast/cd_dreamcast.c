...
static const qboolean fake_cdda = false;
...
static void CDAudio_SetVolume(float volume) {
  ...
  Con_DPrintf("CDAudio: setvolume %d\n", vol);
...
static int CDAudio_GetAudioDiskInfo(void) {
  ...
  da_tracks = 0;
  for (track = TOC_TRACK(toc.first); track <= (int)TOC_TRACK(toc.last); track++) {
...
void CDAudio_Update(void) {
  ...
  if (fake_cdda) {
    //fake_cdda_update();
		...
		if (ret!=ERR_OK) {
			Con_DPrintf("cdrom_get_status failed(%i)\n", ret);
		} else {
			if (status!=CD_STATUS_PLAYING) {
			//	Con_DPrintf("Replay %i %d %d\n",status,playTrack,playLooping);
