...
#ifdef PARANOID
	if (c < -128 || c > 127)
		Con_DPrintf ("MSG_WriteChar: range error");
#endif
...
#ifdef PARANOID
	if (c < 0 || c > 255)
		Con_DPrintf ("MSG_WriteByte: range error");
#endif
...
void MSG_WriteShort (sizebuf_t *sb, int c)
{
	...
	if (c < ((short)0x8000) || c > (short)0x7fff)
		Con_DPrintf ("MSG_WriteShort: range error");
#endif
...
void *SZ_GetSpace (sizebuf_t *buf, int length)
{
	...
	if (buf->cursize + length > buf->maxsize)
	{
		if (!buf->allowoverflow)
			Con_DPrintf ("SZ_GetSpace: overflow without allowoverflow set");
		
		if (length > buf->maxsize)
			Con_DPrintf ("SZ_GetSpace: %i is > full buffer size", length);
...
void COM_CheckRegistered (void)
{
	...
	{
#if WINDED
	Con_DPrintf ("This dedicated server requires a full registered copy of Quake");
#endif
		Con_DPrintf ("Playing shareware version.\n");
		if (com_modified)
			Con_DPrintf ("You must have the registered version to use modified games");
		return;
	}
	...
	for (i=0 ; i<128 ; i++)
		if (pop[i] != (unsigned short)BigShort (check[i]))
			Con_DPrintf ("Corrupted data file.");
#endif
...
void COM_Init (void)
{
	...
	*/
	if ( *(char *)&i == 0x12 )
		...
		Con_DPrintf ("Unsupported endianism.");
			...
			for (i=0 ; i<pak->numfiles ; i++)
				...
				// open a new file on the pakfile
					*file = fopen (pak->filename, "rb");
					if (!*file)
						Con_DPrintf ("Couldn't reopen %s", pak->filename);	
...
pack_t *COM_LoadPackFile (char *packfile)
{
	...
	if (!pak_info)
	{
		...
		if(!pak_info){
			...
			Con_DPrintf("not enough memory for info section!\n");
		}
	}
	...
	|| header.id[2] != 'C' || header.id[3] != 'K')
		Con_DPrintf ("%s is not a packfile", packfile);
	...
	if (numpackfiles > MAX_FILES_IN_PACK)
		Con_DPrintf ("%s has %i files", packfile, numpackfiles);
	...
	if (i)
	{
		...
		while (++i < com_argc)
		{
			...
			if ( !strcmp(COM_FileExtension(com_argv[i]), "pak") )
			{
				...
				if (!search->pack)
					Con_DPrintf ("Couldn't load packfile: %s", com_argv[i]);
			}
			else
