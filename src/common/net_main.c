...
void NET_FreeQSocket(qsocket_t *sock)
{
	...
	else
	{
		...
		if (!s)
			Con_DPrintf ("NET_FreeQSocket: not active\n");
	}
...
static void NET_Port_f (void)
{
	...
	if (n < 1 || n > 65534)
	{
		Con_DPrintf ("Bad value, must be between 1 and 65534\n");
		return;
	}
...
static void PrintSlist(void)
{
	...
	for (n = slistLastShown; n < hostCacheCount; n++)
	{
		if (hostcache[n].maxusers)
			Con_DPrintf("%-15.15s %-15.15s %2u/%2u\n", hostcache[n].name, hostcache[n].map, hostcache[n].users, hostcache[n].maxusers);
		else
			Con_DPrintf("%-15.15s %-15.15s\n", hostcache[n].name, hostcache[n].map);
	}
	slistLastShown = n;
}
...
{
	if (hostCacheCount)
		Con_DPrintf("== end list ==\n\n");
	else
		Con_DPrintf("No Quake servers found.\n\n");
}
...
void NET_Slist_f (void)
{
	...
	if (! slistSilent)
	{
		Con_DPrintf("Looking for Quake servers...\n");
		PrintSlistHeader();
	}
...
int NET_SendMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	if (sock->disconnected)
	{
		Con_DPrintf("NET_SendMessage: disconnected socket\n");
		return -1;
	}
...
void NET_Init (void)
{
	...
	if (i)
	{
		if (i < com_argc-1)
			...
			Con_DPrintf ("NET_Init: you must specify a number after -port");
	}
