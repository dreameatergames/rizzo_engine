...
// turn on color set 1
	if (!fromServer)
		sprintf (text, "%c%s: ", (char)1, save->name);
	else
		sprintf (text, "%c<%s> ", (char)1, hostname.string);
...
void Host_PreSpawn_f (void)
{
	if (cmd_source == src_command)
	{
		Con_DPrintf ("prespawn is not valid from the console\n");
		return;
	}

	if (host_client->spawned)
	{
		Con_DPrintf ("prespawn not valid -- allready spawned\n");
		return;
	}
...
void Host_Spawn_f (void)
{
	...
	if (cmd_source == src_command)
	{
		Con_DPrintf ("spawn is not valid from the console\n");
		return;
	}

	if (host_client->spawned)
	{
		Con_DPrintf ("Spawn not valid -- allready spawned\n");
		return;
	}
...
void Host_Begin_f (void)
{
	if (cmd_source == src_command)
	{
		Con_DPrintf ("begin is not valid from the console\n");
		return;
	}
...
void Host_InitCommands (void)
{
	...
	Cvar_RegisterVariable (&host_mapname);
	host_mapname.string = host_str_mapname;
}
