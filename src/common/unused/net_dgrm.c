...
#ifdef DEBUG
	if (data->cursize == 0)
		Con_DPrintf("Datagram_SendMessage: zero length message\n");

	if (data->cursize > NET_MAXMESSAGE)
		Con_DPrintf("Datagram_SendMessage: message too big %u\n", data->cursize);

	if (sock->canSend == false)
		Con_DPrintf("SendMessage: called with canSend == false\n");
#endif
...
#ifdef DEBUG
	if (data->cursize == 0)
		Con_DPrintf("Datagram_SendUnreliableMessage: zero length message\n");

	if (data->cursize > MAX_DATAGRAM)
		Con_DPrintf("Datagram_SendUnreliableMessage: message too big %u\n", data->cursize);
#endif
...
{
	Con_DPrintf("canSend = %4u   \n", s->canSend);
	Con_DPrintf("sendSeq = %4u   ", s->sendSequence);
	Con_DPrintf("recvSeq = %4u   \n", s->receiveSequence);
	Con_DPrintf("\n");
}
...
void NET_Stats_f (void)
{
	...
	if (Cmd_Argc () == 1)
	{
		Con_DPrintf("unreliable messages sent   = %i\n", unreliableMessagesSent);
		Con_DPrintf("unreliable messages recv   = %i\n", unreliableMessagesReceived);
		Con_DPrintf("reliable messages sent     = %i\n", messagesSent);
		Con_DPrintf("reliable messages received = %i\n", messagesReceived);
		Con_DPrintf("packetsSent                = %i\n", packetsSent);
		Con_DPrintf("packetsReSent              = %i\n", packetsReSent);
		Con_DPrintf("packetsReceived            = %i\n", packetsReceived);
		Con_DPrintf("receivedDuplicateCount     = %i\n", receivedDuplicateCount);
		Con_DPrintf("shortPacketCount           = %i\n", shortPacketCount);
		Con_DPrintf("droppedDatagrams           = %i\n", droppedDatagrams);
	}
...
static void Test_Poll(void)
{
	...
	while (1)
	{
		...
		if (MSG_ReadByte() != CCREP_PLAYER_INFO)
			Con_DPrintf("Unexpected repsonse to Player Info request\n");
...
Error:
	Con_DPrintf("Unexpected repsonse to Rule Info request\n");
Done:
