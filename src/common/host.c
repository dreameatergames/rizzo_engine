...
void Host_Error (char *error, ...)
{
	...
	if (inerror)
		Con_DPrintf ("Host_Error: recursively entered");
	inerror = true;
...
// config.cfg cvars
	if (host_initialized & !isDedicated)
	{
		...
		if (!f)
		{
			Con_DPrintf ("Couldn't write config.cfg.\n");
			return;
		}
...
// Saving options to RAM
		...
		if (!f1)
		{
			Con_DPrintf ("Couldn't write config file.\n");
			return;
		}
...
// Saving config to VM
		...
		Con_DPrintf("Saving options to %s...\n", vmu_path);
...
void Host_ReadConfiguration (void) {
	...
	Con_DPrintf ("Reading settings from %s...\n", path);

	f = fopen(path, "r");

	if (!f) {
		Con_DPrintf ("No config file found in %c-%d.\n", (int)vmu_port.value+'A', (int)vmu_unit.value);
		return;
	}
...
// Checking application name
	...
	{
		Con_DPrintf ("ERROR: not a valid RADQuake game save. (%s)\n", buffer);
		fclose(f);
		return;
	}
...
// make sure all the clients know we're disconnecting
	...
	if (count)
		Con_DPrintf("Host_ShutdownServer: NET_SendToAll failed for %u clients\n", count);
...
// update audio
	...
	if (host_speeds.value)
	{
		...
		Con_DPrintf ("%3i tot %3i server %3i gfx %3i snd\n",
...
void Host_Frame (float time)
{
	...
	Con_DPrintf ("serverprofile: %2i clients %2i msec\n",  c,  m);
}
...
void Host_InitVCR (quakeparms_t *parms)
{
	...
	{
		if (com_argc != 2)
			Con_DPrintf("No other parameters allowed with -playback\n");

		Sys_FileOpenRead("quake.vcr", &vcrFile);
		if (vcrFile == -1)
			Con_DPrintf("playback file not found\n");

		Sys_FileRead (vcrFile, &i, sizeof(int));
		if (i != VCR_SIGNATURE)
			Con_DPrintf("Invalid signature in vcr file\n");
...
void Host_Init (quakeparms_t *parms)
{
	...
	if (parms->memsize < minimum_memory)
		Con_DPrintf ("Only %4.1f megs of memory available, can't execute game", parms->memsize / (float)0x100000);
	...
	if (cls.state != ca_dedicated)
	{
		...
		if (!host_basepal)
			Con_DPrintf ("Couldn't load gfx/palette.lmp");
		host_colormap = (byte *)COM_LoadHunkFile ("gfx/colormap.lmp");
		if (!host_colormap)
			Con_DPrintf ("Couldn't load gfx/colormap.lmp");
...
// keep Con_DPrintf from trying to update the screen
