...
dc:
	...
	========================================"
	@rm -f build_dc/nuquake.elf
	@docker run --rm -v "${PWD}:/${PWD}" -u `id -u`:`id -g` -w "${PWD}" haydenkow/nu_dckos make -j12 -f Makefile.dc
...
test-win: win
	cmd.exe /C "cd D:\Dev\Dreamcast\UB_SHARE\clean_nuQuake\build_win && ""D:\Dev\TDM-GCC-64\bin\gdb.exe"" -q -command debug-win.txt  ""D:\Dev\Dreamcast\UB_SHARE\clean_nuQuake\build_win\nuquake.exe"" "
